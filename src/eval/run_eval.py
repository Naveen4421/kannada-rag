"""Evaluation runner (needs the live stack: Qdrant + models [+ LLM for `rag`]).
    ...
    python -m src.eval.run_eval retrieval [--pool 20] [--top-n 5] [--categories rare_word exact_word]
    python -m src.eval.run_eval rag [--limit 10]
...
rag        C/D. Runs answer_question() per case and reports grounding metrics
              ...
              in src.eval.ragas_eval (LLM-judged); this runner does not replace it.
